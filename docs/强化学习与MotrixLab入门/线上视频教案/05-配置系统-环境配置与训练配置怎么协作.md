...
见 `MotrixLab/motrix_envs/src/motrix_envs/base.py`
...
class EnvCfg:
    ...
    sim_dt: float = 0.01                    # 物理仿真内部时间步长
    max_episode_seconds: float = None        # 每个episode的最大时长（秒）
    ctrl_dt: float = 0.01                   # 控制频率对应的时间间隔
...
见 `MotrixLab/motrix_rl/src/motrix_rl/base.py`
...
class BaseRLCfg:
    ...
    check_point_interval: int = 1000        # 每 1000 步训练保存一次模型检查点
    ...
```
...
**EnvCfg**：在 `EnvCfg` 上继续派生，子类只改差异字段；复杂任务常用嵌套 dataclass 成组管理。

以 `go1-flat-terrain-walk` 为例：
...
**RLCfg**：`PPOCfg` 继承 `BaseRLCfg`；各任务在 `motrix_rl/cfgs.py` 里再继承 `PPOCfg`。
...
**RLCfg**：`Trainer` 先按环境名 + backend 从 registry 实例化默认的 `PPOCfg`，再用 `cfg_override` 做不可变式覆盖：
...
管理大量实验的实质仍是：默认值放在 EnvCfg/RLCfg 子类 → 差异用不同 `@envcfg`/`@rlcfg` 环境名或 backend 类 → 少量动态项用 `cfg_override`/`env_cfg_override`。
...
- 算法与算力：在 `motrix_rl/cfgs.py` 中用 `@rlcfg` 挂不同 `PPOCfg` 子类
- Jax vs Torch 差异：同一 env 注册两个 backend 专用类
